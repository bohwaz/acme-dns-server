...
sudo cp $REALDIR/acme-dns-server.py /usr/bin/acme-dns-server.py
...
sudo cp $REALDIR/acme-dns-server.config /etc/default/acme-dns-server
sudo cp $REALDIR/acme-dns-server.service /etc/systemd/system/acme-dns-server.service
...
if [ -d "~/.acme.sh/" ]; then
    cp $REALDIR/dns_pythondnsd.sh ~/.acme.sh/
fi
